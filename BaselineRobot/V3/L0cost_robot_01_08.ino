/*
Version 01_08 is a checkpoint version following Piwars 2024
All piwars specic code will be removed or incorporated into the main +
Feature development has been frozen to concentrate on demonstrations, optimisation and readability
*/
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// the following include contains the main function and variable declarations
#include "L0cost_robot_dcl.h"
long lastconv = 0;
...
void setup() {
  ...
  //   ledcSetup(flashPWMChannel, PWMFreq, PWMResolution);
  ledcAttach(FLASH_PIN, PWMFreq, PWMResolution);
  ...
  if (debugSerial) { Serial.printf("%s %s %s",__FILE__,__DATE__,__TIME__); }

  if (debugSerial) {Serial.println(getCpuFrequencyMhz());}
  ...
  // use camera defines to setup camera access if required
  // if camera is not setup here it is not available later
  if (execVideo || execGuide) {
    ...
    if (execGuide) {
      ...
      s->set_framesize(s, (framesize_t)2);      //<<<<<<<<<QVGA?
    }
  }
  ...
  if (execPS3) {
    ...
    Ps3.begin(configPS3);
    if (debugSerial) { Serial.println("PS3 services initialised"); }
  }


  // Setup for processing main script file in main Loop
  if (execScript) {
    scriptStatus = 1;
...
/*******************************************************************************************
the loop function runs several recurring routines
   error indicator via flashing inbuilt LED
   switches off timed FLASH illumination
   checks for serial input
   sequences script file processing
   switches off timed motor run 
   schedules camera calibration
   schedules video guidance
   schedule demonstration routines
schedules the reading of command files */
void loop() {
  int status = 0; // general return status variable


  // timing vaiables to measure video guidance routines effectiveness
  long startconv = 0;
  long endconv = 0;
  long enddetect = 0;
  long endoverlay = 0;
  ...
  // calibration routine is called in the loop to run a set number of times
  if (execCalibrate) {
    // if timer expired execute calibartion
    if (calTimer < loopTimer) {
      calTimer = calTimer + 500;
      calCount++;
      // check if maximum number of calibration runs
      if (calCount > calMax || calibrate(calType, calColour)) {
        execCalibrate = 0;
      }
    }
  }
    ...
    //*********************************************************************************
  // if video guidance is in use and blobUpdate or lineUpdate has been requested
  // if viewing on a webpage this should be set automatically, otherwise set by command
  if (execGuide && (blobUpdate || lineUpdate)) {
    ...
    if (frameAvailable == 1) {
      // call line detection processing. 
      ...
      if (execLine) {
        ...
        if (execDemo) {
          //execute line following commands
        }
      }
        ...
        switch (execBlobProcessing) {
          ...
          default:
            ...
            break;
                ...
                if (execDemo) {
          //execute target following commands
        }
