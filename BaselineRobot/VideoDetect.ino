...
long detectColour(long pixel, int colour) {
  ...
  switch (colour) {
    case BLACK:
      ...
      // pixSum = abs(blue255 - green255) + abs(red255 - green255) + abs(red255 - blue255);
      // pixSum = (pixSum * 255)/ (blue255 + green255 + red255);
      // pixSum = 255-pixSum;
      break;
    case RED:
    ...
    case YELLOW:
      ...
      pixSum = guideFrame.data[pixel + 2] + guideFrame.data[pixel + 1] - (abs(guideFrame.data[pixel + 2] - guideFrame.data[pixel + 1]) * 2) - guideFrame.data[pixel] * 4;
      break;
    case GREEN:
    ...
    case WHITE:
      ...
      // pixSum = 255-((abs(guideFrame.data[pixel]-guideFrame.data[pixel+1l] +  abs(guideFrame.data[pixel+2]-guideFrame.data[pixel]+1) +  abs(guideFrame.data[pixel+2]-guideFrame.data[pixel]))) *
      //            (guideFrame.data[pixel] + guideFrame.data[pixel + 1] + guideFrame.data[pixel + 2])/30);
      // long red255 = abs(guideFrame.data[pixel + 2] - pixLuminence) * 255;
      // long green255 = abs(guideFrame.data[pixel + 1] - pixLuminence) * 255;
      // long blue255 = abs(guideFrame.data[pixel + 0] - pixLuminence) * 255;
      //pixSum = 255 * (1- ((abs(blue255 - green255) + abs(red255 - green255) + abs(red255 - blue255)) / (blue255 + green255+ red255)));
      // pixSum = abs(blue255 - green255) + abs(red255 - green255) + abs(red255 - blue255);
      // pixSum = (pixSum * 255)/ (blue255 + green255 + red255);
      // pixSum = 255-pixSum;
      break;
...
long findBlob(int colour) {
  ...
  //
  int pixTest = 0;  // test value of pixel

  for (int pixel = 0; pixel < bytesPerFrame; pixel = pixel + guideFrame.bytes_per_pixel) {
    // only interested in red this time
    pixTest = detectColour(pixel, colour);
    // switch (colour) {
    //   case RED:
    //     pixTest = (guideFrame.data[pixel + 2] + guideFrame.data[pixel + 2] - (guideFrame.data[pixel + 1] + guideFrame.data[pixel]));
    //     break;
    //   case GREEN:
    //     pixTest = (guideFrame.data[pixel + 1] + guideFrame.data[pixel + 1] - (guideFrame.data[pixel + 2] + guideFrame.data[pixel]));
    //     break;
    //   case BLUE:
    //     pixTest = (guideFrame.data[pixel] + guideFrame.data[pixel] - (guideFrame.data[pixel + 1] + guideFrame.data[pixel + 2]));
    //     break;
    // }
  ...
  // De-noise picture, only areas with five adjacent pixels considered
  //
  ...
  for (int pixel = 0; pixel < frameLimit; pixel = pixel + guideFrame.bytes_per_pixel) {
    if (guideFrame.data[pixel]) {
      ...
      if (!pixelBreak) {
        // set pixel as green to indicate its of interest
  ...
  lastBlobX = maxBlobX;
  lastBlobY = maxBlobY;
  Serial.printf("Size = %d,Line = %d, Col = %d \n", lastBlobCount, lastBlobY, lastBlobX);
...
long heatMap(long colour, int background) {
  ...
  for (int pixel = 0; pixel < bytesPerFrame; pixel = pixel + guideFrame.bytes_per_pixel) {
    //  pixTestBlue = map(guideFrame.data[pixel], 0, pixValue, 0, 255);
    //  pixTestGreen = map(guideFrame.data[pixel + 1], 0, pixValue, 0, 255);
    //  pixTestRed = map(guideFrame.data[pixel + 2], 0, pixValue, 0, 255);

    switch (colour) {
      case RED:
        pixTest = (guideFrame.data[pixel + 2] * 2 - (guideFrame.data[pixel + 1] + guideFrame.data[pixel])) / 2;
        // pixTest = pixTestRed; //(pixTestRed * 2 - (pixTestGreen + pixTestBlue)) / 2;
        break;
      case YELLOW:
        pixTest = (guideFrame.data[pixel + 2] + guideFrame.data[pixel + 1] - guideFrame.data[pixel] * 2) / 2;
        // pixTest = (pixTestRed + pixTestGreen)/2 - abs(pixTestRed - pixTestGreen) - pixTestBlue;
        // pixTest = (pixTestRed + pixTestGreen)/2; //- abs(pixTestRed - pixTestGreen) - pixTestBlue;
        break;
      case GREEN:
    ...
    //  pixTest = constrain(pixTest, 0, 255);
...
long findLineBottom(long colour) {
  ...
  int retval = findLine(lineInstancesB, colour, 136, 0, 143, lineThresholdB, 5, 7, 1);
...
long pixelLuminance(int type) {
  long pixLuminance = 0;  // test value of pixel
  long bytesPerFrame = guideFrame.width * guideFrame.height * guideFrame.bytes_per_pixel;
  for (int pixel = 0; pixel < bytesPerFrame; pixel = pixel + guideFrame.bytes_per_pixel) {
    switch (type) {
      case 1:
        pixLuminance = pixLuminance + guideFrame.data[pixel];
        break;
      case 2:
        pixLuminance = pixLuminance + guideFrame.data[pixel + 1];
        break;
      case 3:
        pixLuminance = pixLuminance + guideFrame.data[pixel + 2];
        break;
      default:
        pixLuminance = pixLuminance + guideFrame.data[pixel] + guideFrame.data[pixel + 1] + guideFrame.data[pixel + 2];
    }
  }
  pixLuminance = pixLuminance / bytesPerFrame;
  return pixLuminance;
}
// *********************************************************************************************
